...
└── apps/
    └── <app>/
        └── config.yaml   # Helm chart definitions (optional, pre-installed apps)
```

### root-app.yaml

This is the root `ApplicationSet` that tells ArgoCD how to discover and deploy apps. It uses the git file generator to watch `apps/*/config.yaml`.
...
Any apps you include under `apps/` in the bootstrap repo will be deployed automatically when the cluster is created. For example, a bootstrap repo with:
...
Each app's `config.yaml` defines a Helm chart source:
